...
from scripts.criar_estrutura_bronze import (
...
class BronzeStructureTest(unittest.TestCase):
    def setUp(self):
        self.config = load_config(REPO_ROOT / "config" / "bronze_structure.json")
        self.client = FakeS3Client()

    def test_contract_matches_landing_collections(self):
        self.assertEqual("datalake", self.config.bucket)
        self.assertEqual("ecommerce", self.config.database)
        self.assertEqual("bronze", self.config.layer)
        self.assertEqual(DEFAULT_COLLECTIONS, self.config.tables)
        self.assertEqual(("ingestion_date",), self.config.partition_columns)

    def test_load_config_rejects_invalid_or_duplicate_tables(self):
    ...
    def test_initialize_creates_markers_manifest_and_versioning(self):
        ...
        self.assertEqual([], validate_bronze(self.client, self.config))
    ...
    def test_initialize_is_idempotent(self):
        first = initialize_bronze(self.client, self.config)
        keys_after_first_run = set(self.client.buckets["datalake"])
        puts_after_first_run = self.client.put_count

        second = initialize_bronze(self.client, self.config)
    ...
    def test_validation_reports_missing_marker_and_stale_manifest(self):
        initialize_bronze(self.client, self.config)
        ...
        self.assertEqual(
            ...
            validate_bronze(self.client, self.config),
        )

    def test_manifest_documents_delta_contract(self):
        manifest = json.loads(
            build_structure_manifest(
                self.config,
        ...
        self.assertEqual(["ingestion_date"], manifest["partition_columns"])
        self.assertEqual(10, len(manifest["tables"]))
        self.assertEqual(
            "bronze/ecommerce/itens_pedido/_delta_log/",
